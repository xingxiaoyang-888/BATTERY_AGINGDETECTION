...
package SodiumIonBattery
  ...
  package Functions
    ...
    algorithm
      T_limited := min(380, max(230, T));
      f := exp(Ea/R_gas*(1/T_ref - 1/T_limited));
      annotation (Inline=true);
    end arrheniusFactor;
    ...
    protected
      ...
      SI.Temperature T_limited "限制后的温度（K）";
    algorithm
      T_limited := min(360, max(230, T));
      f := exp(Ea_R/R_gas*(1/T_limited - 1/T_ref));
      annotation (Inline=true);
    ...
    algorithm
      ...
      fSOC := 1 + k_SOC_high*max(0, s - SOC_high)^2 + k_SOC_low*max(0, SOC_low - s)^2;
      fC := 1 + k_C*max(0, C_rate - C_ref);
      f := max(0, fT*fSOC*fC);
      annotation (Inline=true);
    end cycleStress;
    ...
    algorithm
      s := clamp01(SOC);
      lowT := max(0, (T_ref - T)/30);
      highSOC := max(0, (s - SOC_ref)/(1 - SOC_ref + 1e-6));
      highC := max(0, (C_charge - C_ref)/(C_ref + 1e-6));
      risk := lowT*highSOC*(1 + highC);
      annotation (Inline=true);
...
<p>该模型只声明<code>Modelica.Fluid.Interfaces.FluidPort</code>端口，供以后与完整热流体系统连接。本库当前的电池和冷却示例不使用Modelica.Fluid方程，避免把电池包模型与泵、阀、管路网络绑定在一起。</p>
  ...
  package Cells
    ...
    equation
      ...
      SOH = noEvent(max(0.05, 1 - capacityLossOffset - capacityLossCalendar - capacityLossCycle));
      Q_available = max(0.05*data.Q_nominal, data.Q_nominal*SOH);
      der(SOC) = -iTotal/Q_available;
      C_rate = abs(iTotal)/(data.Q_nominal/3600);
      chargeCRate = max(0, -iTotal)/(data.Q_nominal/3600);
      der(EFC) = abs(iTotal)/(2*data.Q_nominal);

      platingRisk = Functions.sodiumPlatingRisk(T, SOC, chargeCRate, aging.T_ref, aging.SOC_high, aging.C_ref);
      calendarFadeRate = aging.calendarFadePerYear_ref/secondsPerYear*
        Functions.calendarStress(T, SOC, aging.T_ref, aging.Ea_calendar, aging.SOC_calendar_ref, aging.k_SOC_calendar);
      cycleFadeRate = der(EFC)*(
      ...
      der(resistanceGrowth) = aging.resistanceGrowthPerCapacityLoss*(calendarFadeRate + cycleFadeRate);

      qOhmic = iTotal*iTotal*R0Base;
      qPolarization = sum(vRC[k]*vRC[k]/max(Rrc[k], 1e-9) for k in 1:3);
      qContact = i*i*contactResistanceAdd;
      qShort = if internalShortResistance < 1e8 then V_cell*V_cell/max(internalShortResistance, 1e-6) else 0;
      qReversible = if data.enableReversibleHeat then -iTotal*T*dUdT else 0;
      qGenerated = qOhmic + qPolarization + qContact + qShort + qReversible;

      data.C_th*der(T) = qGenerated + heatPort.Q_flow;
      heatPort.T = T;

      faultSeverity =
        (if internalShortResistance < 1e8 then min(1, 1/max(internalShortResistance, 1e-6)) else 0)
        + min(1, max(0, contactResistanceAdd/0.005))
        + min(1, max(0, capacityLossOffset/0.20))
        + min(1, max(0, resistanceGrowthOffset/0.50));
